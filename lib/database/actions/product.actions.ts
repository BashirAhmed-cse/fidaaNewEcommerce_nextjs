...
export const getAllProducts = unstable_cache(
  async () => {
    try {
      ...
      const products = await Product.find()
        .populate("category", "name") // only get the `name` field of the category
        .sort({ createdAt: -1 }) // Optional: remove `.sort()` if order doesn't matter
        .lean();
...
//get all review 
export async function getAllProductReviews(limit = 6) {
  try {
    await connectToDatabase();
    ...
    const products = await Product.find({ "reviews.0": { $exists: true } })
      .select("reviews")
      .populate("reviews.reviewBy", "username image") // adjust fields as needed
      .lean();

    // Flatten reviews from all products
    const allReviews = products.flatMap((product: any) =>
      product.reviews.map((review: any) => ({
        id: review._id.toString(),
        name: review.reviewBy?.username || "Anonymous",
        image: review.reviewBy?.image || "/default-avatar.png",
    ...
    // Limit and return
    return { success: true, reviews: sorted.slice(0, limit) };
  } catch (error) {
